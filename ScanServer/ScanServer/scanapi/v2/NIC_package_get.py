...
class NICPackage:
    ...
    def __init__(self, IFACE=None, NEEDPCAP=1000):
        ...
        if WIN:
            ...
            self.prefix = '/'
        self.package_output='pcap' #'screen'/'document'/'all'/'pcap'
    ...
    def package_print(self, packet):
        ...
        if (self.package_output == 'document') or (self.package_output == 'all'):
            outputxt = open(self.filename_txt,'a')
            print(self.filename)
            outputxt.write("\n".join(packet.sprintf("{Raw:%Raw.load%}").split(r"\r\n"))+'\n\n\n')
            outputxt.close()
...
def NICRUN(netname=None, needpcap=1000):
    ...
    if netname:
        ...
        a = NICPackage(IFACE="Realtek PCIe GbE Family Controller", NEEDPCAP=needpcap)
        a.mainrun()
    else:
        print("=======================================================")
        print('请设置网卡名！！！')
