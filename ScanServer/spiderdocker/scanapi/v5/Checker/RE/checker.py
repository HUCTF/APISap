...
def name(resp):
    ...
    if res:
        ...
        # print('检测到疑似中文姓名:')
def address(resp):
    pat="[\u4e00-\u9fa5]{1,3}[省|市|自治区|自治州|县|区|街道|小区|山庄]"
    res= re.findall(pat,resp)
    if res:
        return res
    else:
...
def checker_address(resp):
    a=[]
    a.extend(phone_number(resp))
    a.extend(name(resp))
    a.extend(address(resp))
    return a
def checker_idcode(resp):
    a=[]
    a.extend(id_code(resp))
    return a
def checker_bank_id(resp):
    a=[]
    a.extend(bank_id(resp))
    return a
def checker_emall(resp):
    a=[]
    a.extend(email(resp))
    return a
